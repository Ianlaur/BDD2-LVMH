...
from src.shared.config import (
    ...
    CLUSTERS_K, SKLEARN_RANDOM_STATE, TOP_CONCEPTS_PER_CLUSTER
)
from src.shared.utils import log_stage, set_all_seeds
...
def get_top_concepts_for_cluster(
    ...
    lexicon_dict: Dict,
    top_n: int = TOP_CONCEPTS_PER_CLUSTER
) -> List[str]:
    """
    ...
    # Map to labels from lexicon
    top_labels = []
    for cid in top_concept_ids:
        if cid in lexicon_dict:
            label = lexicon_dict[cid].get("label", cid)
        else:
            # Fallback: clean up the concept_id itself
...
def format_profile_type(concepts: List[str]) -> str:
    ...
    cleaned = []
    for c in concepts[:TOP_CONCEPTS_PER_CLUSTER]:  # Use configured number
...
def segment_clients() -> pd.DataFrame:
    ...
    for cluster_id in sorted(set(labels)):
        ...
        if note_concepts_df is not None and lexicon_dict:
            top_concepts = get_top_concepts_for_cluster(
                cluster_client_ids, note_concepts_df, lexicon_dict
            )
        else:
            top_concepts = []
